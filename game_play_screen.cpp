    ...
    case SDLK_ESCAPE: {
      if (n < 3)
      {
        ...
        if (!isCrashed)
        {
          ...
          world.newObj<Button>(r, audio, LOAD_SPRITE(check_online), LOAD_SPRITE(check_online_down), 130, 230, 371, 18, [this, &r, &audio, n]() {
            newScene = std::make_unique<TitleScreen>(4 + 2 * n, r, audio);
          });
          world.newObj<Button>(r, audio, LOAD_SPRITE(close), LOAD_SPRITE(close_down), 130, 255, 155, 18, [this, &r, &audio, n]() {
            newScene = std::make_unique<TitleScreen>(4 + 2 * n, r, audio);
          });
        }
        isCrashed = true;
